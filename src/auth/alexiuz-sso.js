...
/**
 * Consume an SSO token from the alexiuz auth system.
 * Replicates the logic in alexiuz-auth/callback.php.
 *
 * @param {string} token - 128-char hex token from ?token= query param
 * @param {string} clientIp - requester's IP address
 * @returns {Promise<{alexiuzUserId: number, email: string, username: string|null, role: string}|null>}
 */
async function consumeToken(token, clientIp) {
  if (!token || !/^[a-f0-9]{128}$/.test(token)) {
    return null;
  }
  ...
  await db.execute(
    ...
    [row.user_id, ipHash, now, 'ioproof']
  );
