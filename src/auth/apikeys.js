 ...
 * Returns { fullKey, keyHash, prefix } — fullKey is shown once, keyHash is stored.
 */
function generateApiKey() {
  const raw = crypto.randomBytes(20).toString('hex'); // 40 hex chars
  const fullKey = `iop_live_${raw}`;
  const keyHash = sha256(fullKey);
  const prefix = fullKey.substring(0, 13); // iop_live_XXXX
